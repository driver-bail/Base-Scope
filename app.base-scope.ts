...
const APP = {
  name: "Base Scope (Built for Base)",
...
async function readNetworkSnapshot(address: string) {
  const c = client();
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain validation, and scoped read-only onchain inspection.";
  ...
  btnConnect.onclick = async () => {
    try {
      ...
      const snap = await readNetworkSnapshot(session.address);
      ...
      render([
        ...
        `ETH balance: ${formatEther(snap.balance)} ETH`,
        `Latest block: ${snap.block}`,
  ...
  btnBlock.onclick = async () => {
    try {
      render(["Fetching latest block…"]);
      const b = await readLatestBlock();
      render([
        "Latest Block",
        `Network: ${active.label}`,
